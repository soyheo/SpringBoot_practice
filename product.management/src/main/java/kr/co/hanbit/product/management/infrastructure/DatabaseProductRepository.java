...
public class DatabaseProductRepository  implements ProductRepository {
    ...
    public Product add(Product product) {
        ...
        namedParameterJdbcTemplate
                .update("INSERT INTO products (name, price, amount) " +
    ...
    public Product findById(Long id) {
        ...
        try {
            Product product = namedParameterJdbcTemplate
                    .queryForObject("SELECT id, name, price, amount FROM products WHERE id=:id",
                            namedParameter, new BeanPropertyRowMapper<>(Product.class)
                    );

            return product;
        } catch (EmptyResultDataAccessException exception) {
            throw new EntityNotFoundException("Can not find the product.");
        }
    ...
    public List<Product> findByNameContaining(String name) {
        ...
        List<Product> products = namedParameterJdbcTemplate.query(
                "SELECT * FROM products WHERE name LIKE :name", namedParameter,
                new BeanPropertyRowMapper<>(Product.class)
        );
